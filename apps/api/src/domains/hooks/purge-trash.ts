...
export async function handlePurgeTrash(request: Request): Promise<Response> {
  ...
  try {
    const { photosPurged, projectsPurged, cutoff } = await recordJobRun("purge-trash", async () => {
      ...
      return {
        result: { photosPurged, projectsPurged, cutoff },
        rowsAffected: photosPurged + projectsPurged,
        meta: { photosPurged, projectsPurged, cutoff },
      };
    });
