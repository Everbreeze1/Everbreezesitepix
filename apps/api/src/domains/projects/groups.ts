...
export async function listProjectGroupsService(ctx: AuthedContext) {
  ...
  if (allProjectIds.length) {
    ...
    (
      (ph as Array<{ project_id: string; storage_path: string; image_url: string | null }>) ?? []
    ).forEach((row) => {
...
export async function getProjectGroupService(ctx: AuthedContext, data: any) {
  ...
  if (projectIds.length) {
    ...
    photos.forEach((p) => {
      if (!covers[p.project_id])
        covers[p.project_id] = { path: p.storage_path, image_url: p.image_url };
    });
    ...
    const { data: cls } = await (supabase as any)
      ...
      .order("created_at", { ascending: false });
    const checklists =
      (cls as Array<{ id: string; project_id: string; name: string; created_at: string }>) ?? [];
    const clIds = checklists.map((c) => c.id);
    let itemsByChecklist: Record<string, { total: number; done: number }> = {};
    let fullItemsByChecklist: Record<
      string,
      Array<{ id: string; label: string; completed_at: string | null; position: number }>
    > = {};
    if (clIds.length) {
      ...
      (
        ...
        (fullItemsByChecklist[i.checklist_id] ??= []).push({
          id: i.id,
          label: i.label,
    ...
    const checklistsByProject: Record<
      string,
      Array<{
        id: string;
        name: string;
        ...
        items: Array<{ id: string; label: string; completed_at: string | null; position: number }>;
      }>
    > = {};
    ...
    const { data: tks } = await (supabase as any)
      ...
      //
      // `photo_ids` for the same reason one step further on: a task raised
      // against a set of photos is finished when its photos are, so closing
      // one from here has to close them rather than stamp a status that the
      // next tick on any photo would roll straight back to in_progress.
      .select(
        "id, project_id, title, status, priority, due_date, assignee_email, assignee_user_id, assigned_by, photo_ids, updated_at",
      )
      .in("project_id", projectIds)
...
export async function addProjectToGroupService(ctx: AuthedContext, data: any) {
  ...
  const { error } = await (supabase as any)
    .from("project_group_members")
    .upsert(
      { group_id: data.groupId, project_id: data.projectId },
