...
/*
 * The text renderer moved to lib/pdf-rich.ts. It was private to this file, so
 * the walkthrough PDF grew its own flat, non-paginating copy and shipped a
 * summary with no headings, no bullet markers and a truncated tail. One
 * renderer, two documents.
 */
import {
  drawRichBlocks,
  drawRuns,
...
export async function handleReportPdf(token: string): Promise<Response> {
  ...
  sectionList.forEach((s) =>
    (s.photos ?? []).forEach((p: any) => p?.photo_id && allPhotoIds.add(p.photo_id)),
  );
  ...
  sectionList.forEach((s) =>
    (s.photos ?? []).forEach((p: any) => p?.photo_id && totalPhotoIds.add(p.photo_id)),
  );
  ...
  // -------------------- Sections --------------------
  /*
   * Photo batches are drawn after the text pass. `drawPhotoBatch` is
   * async (it fetches and embeds images) and the page plan is walked
   * synchronously, so the draws are queued against the page they belong
   * to and drained below. pdf-lib does not care about draw order within a
   * page, only that the page object is the right one.
   */
  const photoDraws: Array<{ page: PDFPage; batch: any[]; top: number; availH: number }> = [];
  ...
  if (hasSections) {
     ...
     *
     * Within a planned page the body can still outgrow the sheet; the
     * cursor below adds continuation pages so nothing is lost.
     */
    for (const sec of sectionList) {
      const secPhotos: any[] = (sec.photos ?? []).map((p: any) => ({
        ...p,
        caption: sanitizeCaption(p?.caption),
      }));
      const sectionLabel = sec.title || `Section ${sec.position + 1}`;
      const plan = planSectionPages<any>({
        body: sec.body,
        photos: secPhotos,
        photosPerPage: photosPerPage as 1 | 2 | 3 | 4,
      });

      let page!: PDFPage;
      let py = 0;

      /** Start a fresh sheet with the running header and rule. */
      const startPage = (label: string) => {
        page = pdf.addPage([PAGE_W, PAGE_H]);
        py = PAGE_H - MARGIN;
        drawRunningHeader(page, fonts.regular, safeTitle, label, MARGIN, PAGE_W, MUTED);
        py -= 14;
        page.drawLine({
          start: { x: MARGIN, y: py },
        ...
        // One leading everywhere. This was 26 on a section's first page
        // and 18 on its photo pages, so continuation sheets sat higher.
        py -= 26;
        drawFooter(page, fonts.regular, companyName, MARGIN, MUTED);
        return py;
      };
      ...
      const surface: Surface = {
        get page() {
          ...
          if (Number.isFinite(h) && y - h >= BOTTOM) return { page, y };
          const top = startPage(`${sectionLabel} (cont.)`);
          return { page, y: top };
        },
      };

      plan.forEach((pg, pageIdx) => {
        py = startPage(pageIdx === 0 ? `Section ${sec.position + 1}` : `${sectionLabel} (cont.)`);

        if (pageIdx === 0 && sec.title) {
          // Capped like the report title: an uncapped 26pt heading could
          // eat the whole sheet before the body drew a single line.
          py = drawRuns(surface, [{ text: truncate(String(sec.title), 200), bold: true }], {
            x: MARGIN,
            y: py,
            maxWidth: CONTENT_W,
            size: 26,
        ...
        if (pg.blocks.length) {
          py = drawRichBlocks(surface, pg.blocks, {
            x: MARGIN,
            y: py,
            maxWidth: CONTENT_W,
            baseSize: 11,
    ...
    for (let i = 0; i < legacyPhotos.length; i += photosPerPage) {
      ...
      drawRunningHeader(
        ...
        fonts.regular,
        safeTitle,
        `Photos ${i + 1}-${Math.min(i + photosPerPage, legacyPhotos.length)} of ${legacyPhotos.length}`,
        MARGIN,
        PAGE_W,
        MUTED,
      );
  ...
  // Drain the queued section photo batches onto the pages they were
  // planned for.
  for (const d of photoDraws) {
    await drawPhotoBatch(
      d.page,
      d.batch,
      getImg,
...
async function drawPhotoBatch(
  ...
  if (perPage === 1) {
    ...
    if (!richIsEmpty(sp.caption)) {
      drawRichBlocks(fixedSurface(page), parseRich(sp.caption), {
        ...captionOpts(13),
        x: x0 + imgBoxW + 18,
        y: topY - 6,
        maxWidth: capBoxW,
