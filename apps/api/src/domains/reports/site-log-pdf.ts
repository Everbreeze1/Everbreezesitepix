...
export async function generateSiteLogPdfService(
  ...
  const { supabase, userId } = ctx;
    ...
    const [{ data: photoRows }, { data: profile }] = await Promise.all([
      supabase
        .from("photos")
        .select("id, storage_path, caption, taken_at, project_id")
        .in("id", ids),
      supabase
        .from("profiles")
    ...
    // -------- Cover page --------
    {
      ...
      for (let i = 0; i < stats.length; i++) {
        ...
        page.drawText(s.label, { x: cx - lw / 2, y: rowTop - 12, size: 9, font: fonts.bold, color: SUBTLE });
        const valStr = sanitizeForWinAnsi(truncate(s.value, 32));
        const vw = fonts.bold.widthOfTextAtSize(valStr, 13);
        page.drawText(valStr, { x: cx - vw / 2, y: rowTop - 32, size: 13, font: fonts.bold, color: TEXT });
      }
      y = rowTop - 52;
      ...
      if (projectName) {
        ...
        const cardTop = y - 6;
        page.drawRectangle({
          x: cardX, y: cardTop - cardH, width: cardW, height: cardH,
          color: FAINT, borderColor: BORDER, borderWidth: 0.5,
        });
        const pl = "PROJECT";
        const plW = fonts.bold.widthOfTextAtSize(pl, 9);
        page.drawText(pl, { x: cardX + (cardW - plW) / 2, y: cardTop - 22, size: 9, font: fonts.bold, color: ACCENT });
        const pn = sanitizeForWinAnsi(truncate(projectName, 60));
        const pnW = fonts.bold.widthOfTextAtSize(pn, 15);
        page.drawText(pn, { x: cardX + (cardW - pnW) / 2, y: cardTop - 46, size: 15, font: fonts.bold, color: TEXT });
      }
    ...
    for (let i = 0; i < data.items.length; i += PER_PAGE) {
      ...
      drawRunningHeader(
        page, fonts.regular, sanitizeForWinAnsi(data.title),
        `Photos ${i + 1}-${Math.min(i + PER_PAGE, data.items.length)} of ${data.items.length}`,
        MARGIN, PAGE_W, MUTED,
      );
      let py = PAGE_H - MARGIN - 14;
      page.drawLine({
        start: { x: MARGIN, y: py }, end: { x: PAGE_W - MARGIN, y: py },
        thickness: 0.5, color: BORDER,
      });
      py -= 18;
      ...
      for (let r = 0; r < batch.length; r++) {
        ...
        page.drawRectangle({
          x: MARGIN + 6, y: rowTop - 22, width: bw, height: 16,
          color: ACCENT, borderColor: ACCENT, borderWidth: 0,
        });
        page.drawText(badge, {
          x: MARGIN + 12, y: rowTop - 18, size: 10, font: fonts.bold, color: rgb(1, 1, 1),
        });
        ...
        if (cap) {
          ry = drawWrapped(page, sanitizeForWinAnsi(cap), {
            x: rx, y: ry, width: notesW, size: 13,
            font: fonts.bold, color: TEXT, lineGap: 4,
          });
          ry -= 6;
        }
        ...
        if (meta?.taken_at) {
          ...
          page.drawText(sanitizeForWinAnsi(dt), {
            x: rx, y: ry - 10, size: 10, font: fonts.italic, color: SUBTLE,
          });
          ry -= 18;
        }
        ...
        if (it.notes && it.notes.trim()) {
          ...
          const projectedEnd = drawWrapped(page, noteText, {
            x: rx + 10, y: ry - 2, width: notesW - 12, size: 12,
            font: fonts.regular, color: TEXT, lineGap: 5, dryRun: true,
          });
          const barH = Math.max(20, notesStart - projectedEnd + 2);
          page.drawRectangle({
            x: rx, y: notesStart - barH, width: 3, height: barH, color: ACCENT,
          });
          ry = drawWrapped(page, noteText, {
            x: rx + 10, y: ry - 2, width: notesW - 12, size: 12,
            font: fonts.regular, color: TEXT, lineGap: 5,
          });
          ry -= 10;
        }
        ...
        if (it.todos.length) {
          ...
          page.drawText(heading, {
            x: rx, y: ry - 10, size: 8, font: fonts.bold, color: SUBTLE,
          });
          ry -= 20;
          for (const t of it.todos) {
            ...
            page.drawRectangle({
              ...
              borderColor: t.done ? ACCENT : SUBTLE, borderWidth: 1,
              color: t.done ? ACCENT : undefined,
            });
            if (t.done) {
              page.drawText("v", {
                x: box.x + 1.5, y: box.y + 1, size: 8, font: fonts.bold, color: rgb(1, 1, 1),
              });
            }
            ...
            for (let li = 0; li < wrapped.length; li++) {
              page.drawText(wrapped[li], {
                x: rx + 14, y: ry - 8 - li * 13, size: 11,
                font: fonts.regular, color: t.done ? MUTED : TEXT,
              });
            }
...
interface DrawWrappedOpts {
  ...
  font: PDFFont; color: ReturnType<typeof rgb>; lineGap: number;
  dryRun?: boolean;
}
function drawWrapped(page: PDFPage, text: string, opts: DrawWrappedOpts): number {
  ...
  for (const line of lines) {
    if (!opts.dryRun && line) {
      page.drawText(line, {
        x: opts.x, y: y - opts.size, size: opts.size, font: opts.font, color: opts.color,
      });
    }
    y -= lh;
  }
...
function drawFittedImage(
  page: PDFPage, img: PDFImage | null,
  x: number, y: number, boxW: number, boxH: number,
  border: ReturnType<typeof rgb>, helv: PDFFont, muted: ReturnType<typeof rgb>,
) {
...
function drawRunningHeader(
  ...
  page.drawText(sanitizeForWinAnsi(truncate(left, 60)), {
    x: margin, y: page.getHeight() - margin, size: 9, font, color,
  });
  const rw = font.widthOfTextAtSize(right, 9);
  page.drawText(sanitizeForWinAnsi(right), {
    x: pageW - margin - rw, y: page.getHeight() - margin, size: 9, font, color,
  });
}
...
async function drawCompanyHeader(
  ...
  page.drawText(sanitizeForWinAnsi(name), {
    x: textX, y: topY - 14, size: 14, font: fonts.bold, color: text,
  });
  let cy = topY - 28;
  if (profile?.company_phone) {
    page.drawText(sanitizeForWinAnsi(String(profile.company_phone)), {
      x: textX, y: cy, size: 9, font: fonts.regular, color: muted,
    });
    cy -= 12;
  }
...
function drawFooter(
  page: PDFPage, font: PDFFont, company: string, margin: number,
  color: ReturnType<typeof rgb>,
) {
  page.drawText(sanitizeForWinAnsi(`Generated with SitePix · ${company}`), {
    x: margin, y: 24, size: 8, font, color,
  });
}
...
async function tryEmbedImage(pdf: PDFDocument, url: string): Promise<PDFImage | null> {
  try {
    ...
    try { return await pdf.embedJpg(buf); } catch { /* try png */ }
