...
import { Loader2, Users, Building2, FolderKanban, Image as ImageIcon, ShieldAlert } from "lucide-react";
import { LineChart, Line, ResponsiveContainer, XAxis, YAxis, Tooltip } from "recharts";
import { checkIsPlatformAdmin, getAdminMetrics } from "@/lib/admin.functions";
...
export function AdminPage() {
  ...
  return (
    <div className="min-h-full bg-background p-6 sm:p-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Platform-wide metrics across every team.</p>
              ...
              <div className="mt-4 space-y-3">
                {(["starter", "pro", "team"] as const).map((plan) => (
                  <div key={plan} className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-bold text-foreground">{metrics.teamsByPlan[plan]}</span>
                  </div>
                ))}
                <div className="mt-4 border-t border-border pt-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Active subscriptions</span>
                    <span className="text-sm font-bold text-foreground">{metrics.subscriptions.active}</span>
                  </div>
                  <div className="mt-2 flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Inactive</span>
                    <span className="text-sm font-bold text-foreground">{metrics.subscriptions.inactive}</span>
                  </div>
                </div>
              </div>
            </div>
