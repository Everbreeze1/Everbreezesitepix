...
const HUB_CARDS: HubCardDef[] = [
  {
    ...
    description: "A ready-made project blueprint with checklists, labels and defaults.",
    icon: FolderOpen,
    tint: "from-primary/10 to-primary/[0.03] text-primary",
  },
  {
    id: "checklists",
    title: "Checklist templates",
    description: "Reusable checklists (Pass/Fail, ratings, notes) you attach to projects.",
    icon: ClipboardList,
    tint: "from-emerald-500/15 to-emerald-500/[0.03] text-emerald-600 dark:text-emerald-400",
  },
  {
    id: "workflows",
    title: "Workflow templates",
    description: "Multi-phase standard processes with tasks and gates for consistent delivery.",
    icon: WorkflowIcon,
    tint: "from-violet-500/15 to-violet-500/[0.03] text-violet-600 dark:text-violet-400",
  },
  {
    id: "labels",
    title: "Labels",
    description: "Manage the shared label catalog — names and colors used across projects.",
    icon: Tag,
    tint: "from-amber-500/15 to-amber-500/[0.03] text-amber-600 dark:text-amber-400",
  },
  {
    id: "label-sets",
    title: "Label sets",
    description: "Create a bundle of labels once, then apply the whole set to a project.",
    icon: Tag,
    tint: "from-orange-500/15 to-orange-500/[0.03] text-orange-600 dark:text-orange-400",
  },
  {
    id: "reports",
    title: "Report templates",
    description: "Standardized report structures with cover page, sections and placeholders.",
    icon: FileText,
    tint: "from-sky-500/15 to-sky-500/[0.03] text-sky-600 dark:text-sky-400",
  },
  {
    id: "documents",
    title: "Document templates",
    description: "Word-style rich documents with reusable placeholder fields.",
    icon: FileText,
    tint: "from-rose-500/15 to-rose-500/[0.03] text-rose-600 dark:text-rose-400",
    badge: "Coming soon",
  },
];

function TemplateHub({ onPick }: { onPick: (id: string) => void }) {
  return (
    <div className="mt-2">
        ...
        {HUB_CARDS.map((c) => {
          ...
          return (
            <button
              key={c.id}
              onClick={() => onPick(c.id)}
              className="group relative flex flex-col items-start gap-3 rounded-2xl border border-border/60 bg-card p-5 text-left transition-all hover:-translate-y-0.5 hover:border-border hover:shadow-md"
            >
              <div
                className={`flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br ${c.tint}`}
              >
