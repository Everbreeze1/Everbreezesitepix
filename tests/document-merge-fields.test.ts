...
const PROJECT = { name: "Buddy", street: "9610 Upper Valley Road", city: "Auburn", state: "CA" };
const PROFILE = {
  full_name: "Mike",
 ...
 * client. Both are `.select(...).eq(...).maybeSingle()`, so a table name is all
 * this needs to decide what to hand back.
 */
vi.mock("../apps/api/src/lib/supabase", () => ({
  getSupabaseAdmin: () => ({
    from: (table: string) => ({
      select: () => ({
        eq: () => ({
          maybeSingle: async () => ({ data: table === "projects" ? PROJECT : PROFILE }),
        }),
      }),
    }),
  }),
...
const { resolvePageTokens, tokensToPills, bracketsToFillFields, fieldLabel, SUPPORTED_TOKENS } =
